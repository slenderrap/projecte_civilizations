...
public class Civilization implements Variables {
	...
	public Civilization() { // Lo he creado para poder hacer pruebas BY:Luciano :3
		super();
	...
	public void newRocketLauncherTower(int n) throws ResourceException {
		...
		int baseDamage = (BASE_DAMAGE_ROCKETLAUNCHERTOWER
				+ (getTechnologyAttack() * PLUS_ATTACK_ROCKETLAUNCHERTOWER_BY_TECHNOLOGY) * 1000 / 100);
		for (int i = 0; i < n; i++) {
	...
	public void newPriest(int n) throws ResourceException, BuildingException {
		...
		int baseDamage = 0;
		for (int i = 0; i < n; i++) {
			if (food > FOOD_COST_PRIEST && wood > WOOD_COST_PRIEST && iron > IRON_COST_PRIEST && mana > MANA_COST_PRIEST
					&& army[8].size() < church) {
				...
				for (int j = 0; j < (int) army[i].size(); j++) { // se recorre cada personaje de la unidad
					if (army[i].get(j) instanceof AttackUnity) {
						if (((AttackUnity) army[i].get(j)).isSanctified() == false) {
							// poner armadura +7%
							((AttackUnity) army[i].get(j)).setArmor(((AttackUnity) army[i].get(j)).getArmor()
									+ ((AttackUnity) army[i].get(j)).getArmor() / 100 * PLUS_ARMOR_UNIT_SANCTIFIED);
							// poner damage +7%
							((AttackUnity) army[i].get(j)).setBaseDamage(((AttackUnity) army[i].get(j)).getBaseDamage()
									+ ((AttackUnity) army[i].get(j)).getBaseDamage() / 100 * PLUS_ATTACK_UNIT_SANCTIFIED);
						...
						if (((DefenseUnit) army[i].get(j)).isSanctified() == false) {
							// poner armadura +7%
							((DefenseUnit) army[i].get(j)).setArmor(((DefenseUnit) army[i].get(j)).getArmor()
									+ ((DefenseUnit) army[i].get(j)).getArmor() / 100 * PLUS_ARMOR_UNIT_SANCTIFIED);
							// poner damage +7%
							((DefenseUnit) army[i].get(j)).setBaseDamage(((DefenseUnit) army[i].get(j)).getBaseDamage()
									+ ((DefenseUnit) army[i].get(j)).getBaseDamage() / 100 * PLUS_ATTACK_UNIT_SANCTIFIED);
							// poner sanctified en true
						...
						if (((SpecialUnit) army[i].get(j)).isSanctified() == false) {
							// poner armadura +7%
							((SpecialUnit) army[i].get(j)).setArmor(((SpecialUnit) army[i].get(j)).getArmor()
									+ ((SpecialUnit) army[i].get(j)).getArmor() / 100 * PLUS_ARMOR_UNIT_SANCTIFIED);
							// poner damage +7%
							((SpecialUnit) army[i].get(j)).setBaseDamage(((SpecialUnit) army[i].get(j)).getBaseDamage()
									+ ((SpecialUnit) army[i].get(j)).getBaseDamage() / 100 * PLUS_ATTACK_UNIT_SANCTIFIED);
							// poner sanctified en true
